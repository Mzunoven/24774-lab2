...
int itter_max = 10; // We're only doing 15 itterations 
int itter_length = 10; // How long each itteration is in seconds
...
void loop() {
  ...
  if (itter_count < itter_max) {
    ...
    if (u_ilc_count < num_u_ilc) {
      ...
      Serial.println(std_err); // Finally push the error to serial monitor
      // Calculate new u_ilc values
      for (int a = 0; a < num_u_ilc; a++) {
        u_ilc[a] += Alpha * err[a+1]; 
      }
...
//      Desired = sin(sin_input);
//      sin_input = sin_input + sin_increment;
//      float ERROr = Desired - theta;
//      motorVoltage = -0.5*ERROr;
//      driveMotor();
